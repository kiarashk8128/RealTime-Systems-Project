    ...
    def johnsons_algorithm_multiple_machines_adhoc(self, jobs, num_machines):
        ...
        for index in range(len(ordered_jobs)):
            if ordered_jobs[index, 2] <= ordered_jobs[index, 1]:
                ...
                if num_machines > 3:
                    left_part = np.array(self.johnsons_algorithm_multiple_machines_adhoc(
                        [job_map[i][:num_machines // 2 + 1] for i in ordered_jobs[:index, 0]], num_machines // 2))[:, 0]
                right_part = np.array(self.johnsons_algorithm_multiple_machines_adhoc(
                    [(i,) + job_map[i][num_machines // 2 + 1:] for i in ordered_jobs[index:, 0]],
                    num_machines - num_machines // 2))[:, 0]
